...
static uint8_t reg_buf[PCA9554_REG_MAX];
...
bool pca9554Init(void)
{
  ...
  reg_buf[PCA9554_REG_INPUT]    = 0x00;
  reg_buf[PCA9554_REG_OUTPUT]   = 0xFF;
  reg_buf[PCA9554_REG_POLARITY] = 0x00;
  reg_buf[PCA9554_REG_CONFIG]   = 0xFF;
...
bool pca9554RegRead(PCA9554Reg_t reg, uint8_t *p_data)
{
  ...
  lock();
  if (reg == PCA9554_REG_INPUT)
  {
    ret = i2cReadData(i2c_ch, i2c_addr, &i2c_data, 1, 10);
    if (ret)
    {
      *p_data = i2c_data;
      i2c_retry = 0;
